 ...
 * \mainpage
 *    Contains a test fixture for testing the communication to the actuator with
 * a mock \author Tobit Flatscher (github.com/2b-t)
 */
 ...
 * \brief
 *    Test fixture for testing the communication with a mock of the actual
 * actuator through a (virtual) CAN loopback interface
 */
class ActuatorActuatorMockTest : public ::testing::Test {
 public:
   ...
   *
   * \param[in] ifname
   *    The name of the (virtual) CAN network interface that should be used as a
   * loopback device \param[in] actuator_id The actuator id for the actuator and
   * the actuator mock
   */
  ActuatorActuatorMockTest(std::string const& ifname = "vcan_test",
                           std::uint32_t const actuator_id = 1);
  ActuatorActuatorMockTest(ActuatorActuatorMockTest const&) = delete;
  ActuatorActuatorMockTest& operator=(ActuatorActuatorMockTest const&) =
      default;
