 ...
 * \brief
 *    Counter-part to the actuator and parent class of the mock that can be used
 * for testing the driver over a (virtual) CAN network interface
 */
class ActuatorAdaptor
    : protected myactuator_rmd::CanNode<CanAddressOffset::response,
                                        CanAddressOffset::request> {
 public:
  /**\fn handleRequest
   * \brief
   *    Read a single CAN frame and handle the request by calling one of the
   ...
   */
  [[nodiscard]] virtual myactuator_rmd::GetVersionDateResponse getVersionDate()
      const = 0;
