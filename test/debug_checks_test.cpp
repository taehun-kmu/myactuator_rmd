...
TEST_F(DebugChecksTest, ExceptionMessageQualityTest) {
  ...
  try {
    ...
    EXPECT_TRUE(message.find("Null pointer dereference detected") !=
                std::string::npos);
...
TEST_F(DebugChecksTest, CANSpecificValidationTest) {
  ...
  EXPECT_THROW(DCHECK_LE(0x800, 0x7FF),
               DebugAssertionException);  // Invalid CAN ID (> 0x7FF)
  EXPECT_THROW(DCHECK_GE(-1, 0),
               DebugAssertionException);  // Invalid socket (-1)
