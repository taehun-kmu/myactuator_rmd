...
class PiGains {
 public:
   ...
   *    The integral gain
   */
  constexpr PiGains(std::uint8_t const& kp_ = 0,
                    std::uint8_t const& ki_ = 0) noexcept;
...
class Gains {
 public:
   ...
   */
  constexpr Gains(PiGains const& current_, PiGains const& speed_,
                  PiGains const& position_) noexcept;
  /**\fn Gains
   * \brief
   *    Class constructor
   *
   ...
   */
  constexpr Gains(std::uint8_t const current_kp = 0,
                  std::uint8_t const current_ki = 0,
                  std::uint8_t const speed_kp = 0,
                  std::uint8_t const speed_ki = 0,
                  std::uint8_t const position_kp = 0,
                  std::uint8_t const position_ki = 0) noexcept;
...
constexpr PiGains::PiGains(std::uint8_t const& kp_,
                           std::uint8_t const& ki_) noexcept
    : kp{kp_}, ki{ki_} {
  return;
}

constexpr Gains::Gains(PiGains const& current_, PiGains const& speed_,
                       PiGains const& position_) noexcept
    : current{current_}, speed{speed_}, position{position_} {
  return;
}

constexpr Gains::Gains(std::uint8_t const current_kp,
                       std::uint8_t const current_ki,
                       std::uint8_t const speed_kp, std::uint8_t const speed_ki,
                       std::uint8_t const position_kp,
                       std::uint8_t const position_ki) noexcept
    : current{current_kp, current_ki},
      speed{speed_kp, speed_ki},
      position{position_kp, position_ki} {
  return;
}
