...
class CanNode : public Driver, protected can::Node {
 protected:
  ...
  /**\fn addId
   * \brief
   *    Updates the id as well as the send and receive ids in a consistent
   * manner
   *
  ...
  /**\fn send
   * \brief
   *    Writes a given CAN frame based on the request to the CAN participant
   * with the actuator id actuator_id
   *
   * \param[in] msg
  ...
  inline void send(Message const& msg,
                   std::uint32_t const actuator_id) override;

  /**\fn sendRecv
   * \brief
   *    Writes a given CAN frame based on the request to the actuator with the
   * corresponding id and waits for a corresponding reply
   *
   * \param[in] request
   ...
   *    The response bytes
   */
  [[nodiscard]] inline std::array<std::uint8_t, 8> sendRecv(
      Message const& request, std::uint32_t const actuator_id) override;
 ...
 protected:
   ...
   */
  [[nodiscard]] constexpr std::uint32_t getCanSendId(
      std::uint32_t const actuator_id) noexcept;
   ...
   */
  [[nodiscard]] constexpr std::uint32_t getCanReceiveId(
      std::uint32_t const actuator_id) noexcept;
...
template <std::uint32_t SEND_ID_OFFSET, std::uint32_t RECEIVE_ID_OFFSET>
void CanNode<SEND_ID_OFFSET, RECEIVE_ID_OFFSET>::send(
    Message const& msg, std::uint32_t const actuator_id) {
...
CanNode<SEND_ID_OFFSET, RECEIVE_ID_OFFSET>::sendRecv(
    Message const& request, std::uint32_t const actuator_id) {
  auto const can_send_id{getCanSendId(actuator_id)};
...
CanNode<SEND_ID_OFFSET, RECEIVE_ID_OFFSET>::getCanSendId(
    std::uint32_t const actuator_id) noexcept {
...
template <std::uint32_t SEND_ID_OFFSET, std::uint32_t RECEIVE_ID_OFFSET>
constexpr std::uint32_t
CanNode<SEND_ID_OFFSET, RECEIVE_ID_OFFSET>::getCanReceiveId(
    std::uint32_t const actuator_id) noexcept {
