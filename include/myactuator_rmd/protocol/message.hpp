 ...
 * \file message.hpp
 * \mainpage
 *    Contains base class for any message to be exchanged between driver and
 * actuator \author Tobit Flatscher (github.com/2b-t)
 */
...
class Message {
 ...
 protected:
   ...
   * \param[in] i
   *    The first index of the underlying data array that the value should be
   * set to
   */
  template <typename T,
  ...
  /**\fn getAs
   * \brief
   *    Convert elements of the array to a larger integer value by
   * reinterpret-casting the underlying data
   *
   * \tparam T
   *    Signed or unsigned integer type
   * \param[in] i
   *    The first index of the underlying data array that the value should be
   * extracted from \return The data values located at [i, i + sizeof(T)[ as an
   * integer of type T
   */
  template <typename T,
            typename std::enable_if_t<std::is_integral_v<T>>* = nullptr>
