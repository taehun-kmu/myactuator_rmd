...
using GetAccelerationRequest =
    SingleMotorRequest<CommandType::READ_ACCELERATION>;
using GetControllerGainsRequest =
    SingleMotorRequest<CommandType::READ_PID_PARAMETERS>;
using GetControlModeRequest =
    SingleMotorRequest<CommandType::READ_SYSTEM_OPERATING_MODE>;
...
using GetMotorStatus1Request =
    SingleMotorRequest<CommandType::READ_MOTOR_STATUS_1_AND_ERROR_FLAG>;
using GetMotorStatus2Request =
    SingleMotorRequest<CommandType::READ_MOTOR_STATUS_2>;
using GetMotorStatus3Request =
    SingleMotorRequest<CommandType::READ_MOTOR_STATUS_3>;
using GetMultiTurnAngleRequest =
    SingleMotorRequest<CommandType::READ_MULTI_TURN_ANGLE>;
using GetMultiTurnEncoderPositionRequest =
    SingleMotorRequest<CommandType::READ_MULTI_TURN_ENCODER_POSITION>;
using GetMultiTurnEncoderOriginalPositionRequest =
    SingleMotorRequest<CommandType::READ_MULTI_TURN_ENCODER_ORIGINAL_POSITION>;
using GetMultiTurnEncoderZeroOffsetRequest =
    SingleMotorRequest<CommandType::READ_MULTI_TURN_ENCODER_ZERO_OFFSET>;
using GetSingleTurnAngleRequest =
    SingleMotorRequest<CommandType::READ_SINGLE_TURN_ANGLE>;
using GetSingleTurnEncoderPositionRequest =
    SingleMotorRequest<CommandType::READ_SINGLE_TURN_ENCODER>;
using GetSystemRuntimeRequest =
    SingleMotorRequest<CommandType::READ_SYSTEM_RUNTIME>;
using GetVersionDateRequest =
    SingleMotorRequest<CommandType::READ_SYSTEM_SOFTWARE_VERSION_DATE>;
...
using SetCurrentPositionAsEncoderZeroRequest = SingleMotorRequest<
...
class SetCanIdRequest : public CanIdRequest {
 public:
   ...
   */
  [[nodiscard]] std::uint16_t getCanId() const noexcept;
};
...
class SetAccelerationRequest
 ...
 public:
   ...
   *    The desired acceleration/deceleration in dps with a resolution of 1 dps
   * [100, 60000] \param[in] mode The mode of the desired
   * acceleration/deceleration to be set
   */
  SetAccelerationRequest(std::uint32_t const acceleration,
   ...
   */
  [[nodiscard]] std::uint32_t getAcceleration() const noexcept;
   ...
   */
  [[nodiscard]] AccelerationType getMode() const noexcept;
};
 ...
 */
class SetCanBaudRateRequest
    : public SingleMotorRequest<CommandType::COMMUNICATION_BAUD_RATE_SETTING> {
 public:
   ...
   */
  [[nodiscard]] CanBaudRate getBaudRate() const noexcept;
};
...
using SetControllerGainsPersistentlyRequest =
    SetGainsRequest<CommandType::WRITE_PID_PARAMETERS_TO_ROM>;
using SetControllerGainsRequest =
    SetGainsRequest<CommandType::WRITE_PID_PARAMETERS_TO_RAM>;

/**\class SetPositionAbsoluteRequest
 * \brief
 *    Request for setting the absolute position of the actuator with a given
 * maximum speed
 */
class SetPositionAbsoluteRequest
    : public SingleMotorRequest<
 ...
 public:
  ...
  SetPositionAbsoluteRequest& operator=(SetPositionAbsoluteRequest const&) =
      default;
...
class SetTimeoutRequest
 ...
 public:
   ...
   *
   * \param[in] timeout
   *    The communication interruption protection time setting in milliseconds,
   * 0 if this feature should be de-activated
   */
 ...
 */
class SetTorqueRequest
    : public SingleMotorRequest<CommandType::TORQUE_CLOSED_LOOP_CONTROL> {
 public:
  /**\fn SetTorqueRequest
   * \brief
   *    Class constructor
   ...
   */
  [[nodiscard]] float getTorqueCurrent() const noexcept;
};
 ...
 */
class SetVelocityRequest
    : public SingleMotorRequest<CommandType::SPEED_CLOSED_LOOP_CONTROL> {
 public:
