...
class ActuatorInterface {
 public:
   ...
   *
   * \return
   *    The currently used controller gains for current, speed and position as
   ...
   * \brief
   *    Read the multi-turn encoder position subtracted by the encoder
   * multi-turn zero offset
   *
   * \return
   ...
   * \brief
   *    Read the raw multi-turn encoder position without taking into
   * consideration the multi-turn zero offset
   *
   * \return
   *    The multi-turn encoder position
   */
   ...
   * \return
   *    Feedback control message containing actuator position, velocity, torque
   * and temperature
   */
   ...
   * \brief
   *    Send an absolute position set-point to the actuator additionally
   ...
   * refer to actuator_constants.hpp for the torque constant of your actuator
   * \return
   *    Feedback control message containing actuator position, velocity, torque
   * and temperature
   */
   ...
   *    The desired acceleration/deceleration in dps with a resolution of 1
   * dps/s [100, 60000] For continuous motions the acceleration should be set to
   * the value 0, see
   ...
   * \brief
   *    Set the zero offset (initial position) of the encoder to the current
   * position \warning Motor has be restarted in order for this to become
   * effective
   *
   * \return
   ...
   * \brief
   *    Write the currently used controller gains either to RAM (not persistent
   * after reboot) or ROM (persistent)
   *
   * \param[in] gains
   *    The PI-gains for current, speed and position to be set
   * \param[in] is_persistent
   *    Boolean argument signaling whether the controller gains should be
   * persistent after reboot of the actuator or not \return The currently used
   * controller gains for current, speed and position as unsigned 8-bit integers
   */
  Gains setControllerGains(Gains const& gains,
                           bool const is_persistent = false);

  /**\fn setTimeout
   * \brief
   *    Set the communication interruption protection time setting. The break
   * will be triggered if the communication is interrupted for longer than the
   * set time. The value 0 disables this feature.
   *
   * \param[in] timeout
   *    The desired interruption protection time, 0 in case it should be
   * disabled
   */
