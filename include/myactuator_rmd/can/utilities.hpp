...
/**\fn toTimeval
 * \brief
 *    Function for converting any std::chrono::duration to a Linux timeval
 * struct
 *
 * \tparam Rep
...
[[nodiscard]] constexpr struct ::timeval toTimeval(
    std::chrono::duration<Rep, Period> const& duration) noexcept {
  auto const usec{
      std::chrono::duration_cast<std::chrono::duration<Rep, std::micro>>(
          duration)};
  struct ::timeval tv {};
