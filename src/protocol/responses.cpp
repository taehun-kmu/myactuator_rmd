...
MotorStatus3 GetMotorStatus3Response::getStatus() const noexcept {
  auto const temperature{static_cast<int>(getAs<std::int8_t>(1))};
  auto const current_phase_a{static_cast<float>(getAs<std::int16_t>(2)) *
                             0.01f};
  auto const current_phase_b{static_cast<float>(getAs<std::int16_t>(4)) *
                             0.01f};
  auto const current_phase_c{static_cast<float>(getAs<std::int16_t>(6)) *
                             0.01f};
  return MotorStatus3{temperature, current_phase_a, current_phase_b,
                      current_phase_c};
}
...
std::int16_t GetSingleTurnEncoderPositionResponse::getPosition()
    const noexcept {
  auto const position{getAs<std::int16_t>(2)};
  return position;
}

std::int16_t GetSingleTurnEncoderPositionResponse::getRawPosition()
    const noexcept {
...
std::chrono::milliseconds GetSystemRuntimeResponse::getRuntime()
    const noexcept {
...
std::int32_t SetCurrentPositionAsEncoderZeroResponse::getEncoderZero()
    const noexcept {
